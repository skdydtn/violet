...
#if defined(NDEBUG)
# error "Violet cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Violet Signed Message:\n";
...
{
    // Violet: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Violet: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Violet: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Violet: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Violet: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc1745457e88c7c3a99c56a6dfe723ccf0adb7b020f1dfce7d5a6918557c34832");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Violet Coin. Created by NSSYS. Last modified at 2018-10-22";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Violet: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VioletMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VioletMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VioletMiner(CWallet *pwallet)
{
    printf("VioletMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("violet-miner");
    ...
    try { loop {
        ...
        printf("Running VioletMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VioletMiner, pwallet));
}
